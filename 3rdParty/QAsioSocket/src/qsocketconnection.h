...
public:
    ...
    inline  void wirteData(const QByteArray &data) {
        if (is_open()) {
    ...
    inline  size_t syncWirteData(const QByteArray &data) {
        int sendindex = 0;
        while(sendindex < data.size()){
            if (is_open()) {
                sendindex += socket().write_some(asio::buffer((char *)(data.data())+sendindex,data.size()-sendindex));
            } else {
                return -1;
            }
        }
    ...
    inline  size_t syncReadData(char *data, size_t maxLen, bool peek = false) {
        if (is_open()) {
            return socket().receive(asio::buffer(data, maxLen),
                                    peek ? asio::socket_base::message_peek : 0);
        } else {
            return -1;
        }
    }
    ...
    inline void do_read() {
        ...
        if (useSSL)
            ...
            socket().async_read_some(asio::buffer(buffer_->data(),buffer_->capacity()),
                                     stand_->wrap(std::bind(&QSocketConnection::readHandler,shared_from_this(),
                                                            std::placeholders::_1,std::placeholders::_2)));
