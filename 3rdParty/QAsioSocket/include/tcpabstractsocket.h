...
public slots:
    ...
    inline size_t syncWrite(const QByteArray & data){
        return syncSendData(data);
    }
    inline size_t syncRead(char *data, size_t maxLen){
        return syncReadData(data, maxLen);
    }
    inline size_t peek(char *data, size_t maxLen){
        return syncReadData(data, maxLen, true);
    }
...
protected:
    ...
    virtual size_t syncSendData(const QByteArray &data) = 0;
    virtual size_t syncReadData(char *data, size_t maxLen, bool peek = false) = 0;
